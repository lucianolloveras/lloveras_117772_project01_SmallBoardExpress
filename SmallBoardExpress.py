from graphics import  *
import random

#Se crea la ventana, el "dado" y se inicializan variables
#Ventana
win = GraphWin('Juego',1400,800, autoflush = False)
#Dado
dado = Rectangle(Point(50,600), Point(200,700)).draw(win)
boton = Text(dado.getCenter(),"Tirar dado Jugador 1")
salida = Text(Point(400,650),"")
#Inicialización de variables
lanzamiento1 = 0
nombre = ""

#Jugador número 1
#Se crea el modelo de la ficha del jugador número 1.
#Se dibuja en pantalla y se le pasan valores como los colores, el nombre, tipo de letra, etc.
p1 = Rectangle(Point(30,30), Point(100,100))
p1.draw(win)
p1.move(100,200)
p1.setFill("red")
centro1 = p1.getCenter()
label1 = Text(centro1, "Jugador 1")
label1.setSize(9)
label1.setStyle("bold")
label1.draw(win)
    
#Jugador número 2
#Se crea el modelo de la ficha del jugador número 2.
#Se dibuja en pantalla y se le pasan valores como los colores, el nombre, tipo de letra, etc.
p2 = Circle(Point(500,250),40)
p2.draw(win)
p2.move(-420,15)
p2.setFill("grey")
centro2 = p2.getCenter()
label2 = Text(centro2, "Jugador 2")
label2.setSize(9)
label2.setStyle("bold")
label2.draw(win)

#Función correspondiente al dado
def dice():
    boton.setSize(10)
    boton.setStyle("bold")
    boton.setFace("arial")
    dado.setFill("white")
    boton.draw(win)
    cara = Text(Point(350,650),"Número :")
    cara.setStyle("italic")
    cara.draw(win)
    salida.setStyle("italic")
    salida.draw(win)

#Función correspondiente al lanzamiento del jugador 1
def lanzamientojugador1():
    global lanzamiento1
    global nombre   
    inicio = 1
    while inicio > 0:
        p = win.getMouse()
        if(50 <= p.getX() <= 200) and (600 <= p.getY() <= 700): 
            inicio = 0
    if inicio == 0:        
        lanzamiento1 = random.randint(1,6)
        salida.setText(lanzamiento1)
        boton.setText("Tirar dado Jugador 2")         
        nombre = "Jugador 2"
    return (lanzamiento1,salida,boton,nombre)
        
#Función correspondiente al lanzamiento del jugador 2
def lanzamientojugador2():
    global lanzamiento1
    global nombre
    inicio = 1
    while inicio > 0:
        p = win.getMouse()
        if(50 <= p.getX() <= 200) and (600 <= p.getY() <= 700): 
            inicio = 0
    if inicio == 0:        
        lanzamiento1 = random.randint(1,6)
        salida.setText(lanzamiento1)
        boton.setText("Tirar dado Jugador 1")         
        nombre = "Jugador 1"  
    return (lanzamiento1,salida,boton,nombre)

#Función correspondiente al movimiento en pantalla del jugador 1
def movimiento1():
    p1.undraw()
    label1.undraw()
    p1.draw(win)
    label1.draw(win)

#Función correspondiente al movimiento en pantalla del jugador 2
def movimiento2():
    p2.undraw()
    label2.undraw()
    p2.draw(win)
    label2.draw(win)

#Función correspondiente al armado del tablero
def tablero():
    tablero_lineas = [Line(Point(0,180), Point(1500,180)),Line(Point(0,360), Point(1500,360)),
    Line(Point(0,540), Point(1500,540)), Text(Point(120,340),"Inicio de juego"),
    Text(Point(1280,520),"Final del juego")]

    count = 0

    for i in tablero_lineas:
        tl = i
        tl.draw(win)
        count = count + 1
        if (count > 3):
            msj1 = i
            msj1.setStyle("bold")
            msj1.setFace("arial")
            

    rectangulos = [Rectangle(Point(280,180),Point(500,360)),Rectangle(Point(500,180),Point(720,360)),
    Rectangle(Point(720,180),Point(940,360)),Rectangle(Point(940,180),Point(1160,360)),
    Rectangle(Point(280,360),Point(500,540)),Rectangle(Point(500,360),Point(720,540)),
    Rectangle(Point(720,360),Point(940,540)),Rectangle(Point(940,360),Point(1160,540)),
    Rectangle(Point(1160,180),Point(1400,360)),Rectangle(Point(0,360),Point(280,540))]

    colores = ["green","coral","cyan","light blue","orange","gold","light salmon",
    "white","firebrick","lemon chiffon"]
    
    j = 0

    for i in rectangulos:
        a = i
        a.draw(win)
        a.setFill(colores[j])
        j = j + 1
    
    tablero = [Text(Point(380,340),"1"),Text(Point(600,340),"2"),Text(Point(820,340),"3"),
    Text(Point(1040,340),"4"),Text(Point(1280,340),"5"),Text(Point(120,520),"6"),Text(Point(380,520),"7"),
    Text(Point(600,520),"8"),Text(Point(820,520),"9"),Text(Point(1040,520),"10")]

    for i in tablero:
        b = i
        b.setStyle("bold")
        b.draw(win)



def main():

    tablero()
    dice()

    #Se define la variable pos que representara la posición que ocupa la ficha dentro del tablero.
    pos = 0
    pos2 = 0
        
    while (pos < 11) or (pos2 < 11):
        #Jugador 01
        #Siempre la primera posición va a ser equivalente a cero porque es el punto de partida.
        #Se realiza el "lanzamiento" aleatorio y se avanza a la casilla correspondiente.
        if (pos == 0):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(280,0)
                label1.move(280,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(500,0)
                label1.move(500,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(720,0)
                label1.move(720,0)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(940,0)
                label1.move(940,0)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):
                p1.move(1180,0)
                label1.move(1180,0)
                pos = pos + 5
                movimiento1()
            elif (lanzamiento1 == 6):
                p1.move(0,180)
                label1.move(0,180)
                pos = pos + 6
                movimiento1()


        #Posición 1 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 1):
            lanzamientojugador1()
            if (nombre == "Jugador 1") and (lanzamiento1 == 1):
                p1.move(220,0)
                label1.move(220,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(440,0)
                label1.move(440,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(660,0)
                label1.move(660,0)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(900,0)
                label1.move(900,0)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):    
                p1.move(-280,180)
                label1.move(-280,180)
                pos = pos + 5
                movimiento1()
            elif (lanzamiento1 == 6):
                p1.move(0,180)
                label1.move(0,180)
                pos = pos + 6
                movimiento1()

        #Posición 2 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 2):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(220,0)
                label1.move(220,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(440,0)
                label1.move(440,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(680,0)
                label1.move(680,0)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(-500,180)
                label1.move(-500,180)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):    
                p1.move(-220,180)
                label1.move(-220,180)
                pos = pos + 5
                movimiento1()
            elif (lanzamiento1 == 6):
                p1.move(0,180)
                label1.move(0,180)
                pos = pos + 6        
                movimiento1()

        #Posición 3 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 3):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(220,0)
                label1.move(220,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(460,0)
                label1.move(460,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(-720,180)
                label1.move(-720,180)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(-440,180)
                label1.move(-440,180)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):    
                p1.move(-220,180)
                label1.move(-220,180)
                pos = pos + 5
                movimiento1()
            elif (lanzamiento1 == 6):
                p1.move(0,180)
                label1.move(0,180)
                pos = pos + 6     
                movimiento1()

        #Posición 4 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 4):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(240,0)
                label1.move(240,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(-940,180)
                label1.move(-940,180)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(-660,180)
                label1.move(-660,180)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(-440,180)
                label1.move(-440,180)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):    
                p1.move(-220,180)
                label1.move(-220,180)
                pos = pos + 5
                movimiento1()
            elif (lanzamiento1 == 6):
                p1.move(0,180)
                label1.move(0,180)
                pos = pos + 6                       
                movimiento1()

        #Posición 5 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 5):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(-1180,180)
                label1.move(-1180,180)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(-900,180)
                label1.move(-900,180)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(-680,180)
                label1.move(-680,180)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(-460,180)
                label1.move(-460,180)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):    
                p1.move(-240,180)
                label1.move(-240,180)
                pos = pos + 5
                movimiento1()
            elif (lanzamiento1 == 6):
                p1.move(0,180)
                label1.move(0,180)
                pos = pos + 6
                movimiento1()
                break                       

        #Posición 6 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 6):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(280,0)
                label1.move(280,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(500,0)
                label1.move(500,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(720,0)
                label1.move(720,0)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(940,0)
                label1.move(940,00)
                pos = pos + 4
                movimiento1()
            elif (lanzamiento1 == 5):    
                p1.move(1180,0)
                label1.move(1180,0)
                pos = pos + 5
                movimiento1()
                break
            elif (lanzamiento1 == 6):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0

        #Posición 7 - para el Jugador 01
        if (nombre == "Jugador 1") and (pos == 7):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(220,0)
                label1.move(220,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(440,0)
                label1.move(440,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(660,0)
                label1.move(660,0)
                pos = pos + 3
                movimiento1()
            elif (lanzamiento1 == 4):
                p1.move(900,0)
                label1.move(900,00)
                pos = pos + 4
                movimiento1()
                break
            elif (lanzamiento1 == 5):    
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 6):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0                       

        #Posición 8 - para el Jugador 01
        if (pos == 8):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(220,0)
                label1.move(220,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(440,0)
                label1.move(440,0)
                pos = pos + 2
                movimiento1()
            elif (lanzamiento1 == 3):
                p1.move(680,0)
                label1.move(680,0)
                pos = pos + 3
                movimiento1()
                break
            elif (lanzamiento1 == 4):
                p1.move(0,0)
                label1.move(0,00)
                pos = pos + 0
            elif (lanzamiento1 == 5):    
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 6):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0  

        #Posición 9 - para el Jugador 01    
        if (nombre == "Jugador 1") and (pos == 9):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(220,0)
                label1.move(220,0)
                pos = pos + 1
                movimiento1()
            elif (lanzamiento1 == 2):
                p1.move(460,0)
                label1.move(460,0)
                pos = pos + 2
                movimiento1()
                break
            elif (lanzamiento1 == 3):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 4):
                p1.move(0,0)
                label1.move(0,00)
                pos = pos + 0
            elif (lanzamiento1 == 5):    
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 6):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0  

        #Posición 10 - para el Jugador 01    
        if (nombre == "Jugador 1") and (pos == 10):
            lanzamientojugador1()
            if (lanzamiento1 == 1):
                p1.move(240,0)
                label1.move(240,0)
                pos = pos + 1
                movimiento1()
                break
            elif (lanzamiento1 == 2):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 3):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 4):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 5):    
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0
            elif (lanzamiento1 == 6):
                p1.move(0,0)
                label1.move(0,0)
                pos = pos + 0



        #Jugador02
        if (pos2 == 0):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(250,0)
                label2.move(250,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(470,0)
                label2.move(470,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(690,0)
                label2.move(690,0)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(910,0)
                label2.move(910,0)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):
                p2.move(1130,0)
                label2.move(1130,0)
                pos2 = pos2 + 5
                movimiento2()
            elif (lanzamiento1 == 6):
                p2.move(0,180)
                label2.move(0,180)
                pos2 = pos2 + 6
                movimiento2()
        
        #Posición 1 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 1):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(440,0)
                label2.move(440,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(660,0)
                label2.move(660,0)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(880,0)
                label2.move(880,0)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):    
                p2.move(-250,180)
                label2.move(-250,180)
                pos2 = pos2 + 5
                p2.undraw()
                movimiento2()
            elif (lanzamiento1 == 6):
                p2.move(0,180)
                label2.move(0,180)
                pos2 = pos2 + 6
                movimiento2()
        
        #Posición 2 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 2):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(440,0)
                label2.move(440,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(660,0)
                label2.move(660,0)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(-470,180)
                label2.move(-470,180)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):    
                p2.move(-220,180)
                label2.move(-220,180)
                pos2 = pos2 + 5
                movimiento2()
            elif (lanzamiento1 == 6):
                p2.move(0,180)
                label2.move(0,180)
                pos2 = pos2 + 6        
                movimiento2()
        
        #Posición 3 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 3):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(440,0)
                label2.move(440,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(-690,180)
                label2.move(-690,180)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(-440,180)
                label2.move(-440,180)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):    
                p2.move(-220,180)
                label2.move(-220,180)
                pos2 = pos2 + 5
                movimiento2()
            elif (lanzamiento1 == 6):
                p2.move(0,180)
                label2.move(0,180)
                pos2 = pos2 + 6
                movimiento2()
        
        #Posición 4 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 4):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(-910,180)
                label2.move(-910,180)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(-660,180)
                label2.move(-660,180)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(-440,180)
                label2.move(-440,180)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):    
                p2.move(-220,180)
                label2.move(-220,180)
                pos2 = pos2 + 5
                movimiento2()
            elif (lanzamiento1 == 6):
                p2.move(0,180)
                label2.move(0,180)
                pos2 = pos2 + 6
                movimiento2()
        
        #Posición 5 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 5):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(-1130,180)
                label2.move(-1130,180)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(-880,180)
                label2.move(-880,180)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(-660,180)
                label2.move(-660,180)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(-440,180)
                label2.move(-440,180)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):    
                p2.move(-220,180)
                label2.move(-220,180)
                pos2 = pos2 + 5
                movimiento2()
            elif (lanzamiento1 == 6):
                p2.move(0,180)
                label2.move(0,180)
                pos2 = pos2 + 6
                movimiento2()
                break                       
        
        #Posición 6 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 6):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(250,0)
                label2.move(250,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(470,0)
                label2.move(470,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(690,0)
                label2.move(690,0)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(910,0)
                label2.move(910,00)
                pos2 = pos2 + 4
                movimiento2()
            elif (lanzamiento1 == 5):    
                p2.move(1130,0)
                label2.move(1130,0)
                pos2 = pos2 + 5
                movimiento2()
                break
            elif (lanzamiento1 == 6):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0 
                                      
        
        #Posición 7 - para el Jugador 02
        if (nombre == "Jugador 2") and  (pos2 == 7):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(440,0)
                label2.move(440,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(660,0)
                label2.move(660,0)
                pos2 = pos2 + 3
                movimiento2()
            elif (lanzamiento1 == 4):
                p2.move(900,0)
                label2.move(900,00)
                pos2 = pos2 + 4
                movimiento2()
                break
            elif (lanzamiento1 == 5):    
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 6):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0                       
        
        #Posición 8 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 8):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(440,0)
                label2.move(440,0)
                pos2 = pos2 + 2
                movimiento2()
            elif (lanzamiento1 == 3):
                p2.move(680,0)
                label2.move(680,0)
                pos2 = pos2 + 3
                movimiento2()
                break
            elif (lanzamiento1 == 4):
                p2.move(0,0)
                label2.move(0,00)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 5):    
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 6):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0  
        
        #Posición 9 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 9):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
            elif (lanzamiento1 == 2):
                p2.move(440,0)
                label2.move(440,0)
                pos2 = pos2 + 2
                movimiento2()
                break
            elif (lanzamiento1 == 3):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 4):
                p2.move(0,0)
                label2.move(0,00)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 5):    
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 6):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0  
        
        #Posición 10 - para el Jugador 02
        if (nombre == "Jugador 2") and (pos2 == 10):
            lanzamientojugador2()
            if (lanzamiento1 == 1):
                p2.move(220,0)
                label2.move(220,0)
                pos2 = pos2 + 1
                movimiento2()
                break
            elif (lanzamiento1 == 2):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 3):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 4):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 5):    
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0
            elif (lanzamiento1 == 6):
                p2.move(0,0)
                label2.move(0,0)
                pos2 = pos2 + 0

        update()

    
    #Se muestra en la ventana un mensaje con el jugador que gano la partida
    if (pos == 11):
        mensajefinal = Text(Point(1000,650),"Ganador : Jugador 1")
        mensajefinal.draw(win)
        mensajefinal.setStyle("bold")
    elif (pos2 == 11):
        mensajefinal = Text(Point(1000,650),"Ganador : Jugador 2")    
        mensajefinal.draw(win)
        mensajefinal.setStyle("bold")
    
    #Se espera que se realice un click para finalizar/cerrar la ventana
    mensajecerrar = Text(Point(700,700),"Haga un click para cerrar la ventana")
    mensajecerrar.draw(win)
    mensajecerrar.setSize(15)
    win.getMouse()
    win.close()

main()    


